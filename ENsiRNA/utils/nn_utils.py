...
class EdgeConstructor:
    def __init__(self, bos_idx,bom_idx, atom_pos_pad_idx) -> None:
        self.bos_idx=bos_idx
        self.bom_idx=bom_idx
        self.atom_pos_pad_idx = atom_pos_pad_idx
        self.pad_index = VOCAB.symbol_to_idx('N')
    ...
    def manual_cumsum(self,tensor, dim=-1):
        ...
        result = torch.zeros_like(tensor).to(device=tensor.device)
...
# Initialize the result tensor with zeros
        lengths = torch.zeros(batch_id.max() + 1, dtype=ones.dtype).to(device=batch_id.device)
...
# Use scatter_add to perform the summation
    ...
    def _prepare(self, S, batch_id) -> None:
        ...
        is_global = sequential_or(S == self.bos_idx, S == self.bom_idx)
        ...
        self.pad_mask = S == VOCAB.symbol_to_idx('N')
        # segment ids
    ...
    @torch.no_grad()
    def construct_edges(self, X, S,sec_pos,chain, batch_id, k_neighbors, atom_pos):
        '''
        ...
        #row, col = inner_edges
        #inner_edges = inner_edges[:,(sec_pos[row] >=0) & (sec_pos[col] >=0)]

        row, col = self.row, self.col
        inner_edges = torch.stack([row[self.not_global_edges], col[self.not_global_edges]])
        row, col = inner_edges
        inner_edges = inner_edges[:,sec_pos[row] == sec_pos[col]]
        
        # 移除chain为sense sirna和chain为mrna之间的边
        #row, col = inner_edges
        #mask = ~((chain[row] == 1) & (chain[col] == 3) | (chain[row] == 3) & (chain[col] == 1))
        #inner_edges = inner_edges[:, mask]
       


        # edges between global nodes and normal/global nodes
        global_normal = self._construct_global_edges()
        row, col = global_normal
        mask = ((chain[row] == 1) & (chain[col] == 0)) | ((chain[row] == 0) & (chain[col] == 1) | ((chain[row] == 3) & (chain[col] == 2)) | ((chain[row] == 2) & (chain[col] == 3)))
        
        global_normal = global_normal[:, mask]
        #row, col = global_normal
        #mask = ((chain[row] == 3) & (chain[col] == 2) | (chain[row] == 2) & (chain[col] == 3))
        #global_normal = global_normal[:, mask]
        ...
        row, col = seq_edges
        mask = ~((chain[row] == 1) & (chain[col] == 3) | (chain[row] == 3) & (chain[col] == 1))
        seq_edges = seq_edges[:, mask]

        # construct  edges
        #ctx_edges = torch.cat([inner_edges,  seq_edges], dim=1) 
        ctx_edges = torch.cat([inner_edges, global_normal, seq_edges], dim=1) # [2, E]
        row, col = ctx_edges
        pad_mask = (self.pad_mask[row] == False) & (self.pad_mask[col] == False)
        ctx_edges = ctx_edges[:,pad_mask]
        row, col = ctx_edges
        simask = (chain[row] !=1) & (chain[col] !=1) & (chain[col] !=0) & (chain[row] !=0)
        sirna_edges = ctx_edges[:,simask]
        mrna_edges = ctx_edges[:,(chain[row] == 1) | (chain[col] ==1) | (chain[row] == 0) | (chain[col] == 0)]
        self._reset_buffer()
        #print(sirna_edges,'sirna_edges')
        #print(mrna_edges,'mrna_edges')
        #print(global_normal,'global_normal')
        #print(inner_edges,'inner_edges')
        #print(global_normal,'global_normal')
        #print(seq_edges,'seq_edges')
        return sirna_edges,mrna_edges
...
class NucleicAcidEmbedding(nn.Module):
    ...
    def __init__(self, num_rna_type, num_atom_type, num_atom_pos, rna_embed_size, atom_embed_size,
                 atom_pad_id=VOCAB.get_atom_pad_idx(), relative_position=True, max_position=120,tf_dim=512,dis_dim=7):  # max position (30+30)
        super().__init__()
        num_rna_type = 10
        self.rna_embedding = nn.Embedding(num_rna_type, 64 -3) #rna_embed_size,
        if relative_position:
            self.rna_pos_embedding = SinusoidalPositionEmbedding(rna_embed_size)  # relative positional encoding
        else:
            self.rna_pos_embedding = nn.Embedding(max_position, 64 -3)
        ...
        self.fm_embedding = nn.Linear(640,32)
...
class NucleicAcidFeature(nn.Module):
    def __init__(self, embed_size, relative_position=True, edge_constructor=EdgeConstructor, backbone_only=False) -> None:
        ...
        self.bom_idx = VOCAB.symbol_to_idx(VOCAB.BOM)
        ...
        for i in range(len(VOCAB)):
            if i == self.bos_idx or i == self.bom_idx or i == self.mask_idx:
                # global nodes
        ...
        # edge constructor
        self.edge_constructor = edge_constructor(self.bos_idx, self.bom_idx, self.atom_pos_pad_idx)

    def _is_global(self, S):
        return sequential_or(S == self.bos_idx, S == self.bom_idx) 
    ...
    @torch.no_grad()
    def construct_edges(self, X, S,sec_pos,chain, batch_id, k_neighbors, atom_pos=None, ):
        ...
        ctx_edges = self.edge_constructor.construct_edges(
            X, S, sec_pos,chain,batch_id, k_neighbors, atom_pos)
...
class SeparatedNucleicAcidFeature(NucleicAcidFeature):
    ...
    def forward(self, X, S, batch_id, k_neighbors, rna_pos,sec_pos,chain,energys,single_embeddings,duplex_embeddings):
        ...
        sirna_edges,mrna_edges = self.construct_edges(
            X, S,sec_pos,chain, batch_id, k_neighbors, atom_pos=atom_pos)
        

        #smask = torch.any(atom_mask != torch.tensor([0, 0, 0]).to('cuda'), dim=1)

        single_embeddings = self.rna_embedding.fm_embedding(single_embeddings)
        duplex_embeddings = self.rna_embedding.fm_embedding(duplex_embeddings)
        FM = torch.cat([single_embeddings,duplex_embeddings],dim=-1)
        # rna embedding
        ...
        #H = H + FM
        #print(H.shape,pos_embedding.shape,FM.shape,energys.shape)
        H = H + pos_embedding #+ FM #+energys.unsqueeze(-1)   #+rna_pp #+ chain_embedding
        H = torch.cat([H,FM],dim=-1)
        H = torch.cat([H,energys],dim=-1)
        ...
        return H,(sirna_edges,mrna_edges), (atom_embedding, atom_weights)
