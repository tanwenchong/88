...
class AMEGNN(nn.Module):
    ...
    def __init__(self, in_node_nf, hidden_nf, out_node_nf, n_channel, channel_nf,
                 radial_nf, in_edge_nf=0, act_fn=nn.SiLU(), n_layers=3,
        ...
        self.dense = dense
        if dense:
            self.linear_out = nn.Linear(self.hidden_nf * (self.n_layers + 1), out_node_nf)
        else:
    ...
    def forward(self, h, x, sirna_edge_index, mrna_edge_index, channel_attr, channel_weights, ctx_edge_attr=None):
        h = self.linear_in(h)
        h = self.dropout(h)

        ctx_states, ctx_coords, attention = [], [], []
        for i in range(0, self.n_layers):
            h = self._modules[f'gcl_{i}'](
                h, sirna_edge_index, mrna_edge_index, x, channel_attr, channel_weights,
                edge_attr=ctx_edge_attr)
          
            ctx_states.append(h)
            ctx_coords.append(x)
            #attention.append(edge_attention)
        mid_h = h
        ...
        #if edge_attention != None:
        #    attention = torch.mean(edge_attention,dim=1)
        #else:
        #    attention = None
        h = self.dropout(h)
        h = self.linear_out(h)
...
class AM_E_GCL(nn.Module):
    ...
    def edge_model(self, msource, mtarget, ssource, starget, radial, edge_attr):
        '''
        ...
        if edge_attr is None:  # Unused.
            mout = torch.cat([msource, mtarget], dim=1)
            sout = torch.cat([ssource, starget, radial], dim=1)
            #out = torch.cat([source, target], dim=1)
        else:
            out = torch.cat([source, target, radial, edge_attr], dim=1)
        mout = self.edge_mlp1(mout)
        sout = self.edge_mlp2(sout)
        #print(mout.shape,sout.shape)
        out = torch.cat([mout, sout], dim=0)
    ...
    def forward(self, h, sirna_edge_index, mrna_edge_index, coord, channel_attr, channel_weights,
        ...
        '''
        srow, scol = sirna_edge_index
        mrow, mcol = mrna_edge_index

        radial, coord_diff = coord2radial(sirna_edge_index, coord, channel_attr, channel_weights, self.radial_linear)
        #mradial, m_coord_diff = coord2radial(medge_index, coord, channel_attr, channel_weights, self.radial_linear)

        edge_feat,att_val = self.edge_model(h[mrow], h[mcol],h[srow], h[scol], radial, edge_attr)  # [n_edge, hidden_size]
         # [n_edge, hidden_size]
        #coord = self.coord_model(coord, edge_index, coord_diff, edge_feat, channel_weights)    # [bs * n_node, n_channel, d]
        edge_index = torch.cat([mrna_edge_index,sirna_edge_index],dim=-1)
